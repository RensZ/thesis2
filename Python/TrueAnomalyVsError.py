...
def f(dir_output, dir_plots, body, no_arcs, useRSW):
    ...
    for consider in range(c):
        ...
        plt.tight_layout()
        plt.savefig(dir_plots+"ErrorsVSTrueAnomaly"+saveRSW+savestring)

        if not useRSW:
            save_array = np.vstack((arcnumbers, t, TA, errors[:,1], errors[:,2], errors[:,3], errors[:,4], errors[:,5], errors[:,6])).T
            averages_filename = "/home/rens/tudatBundle/tudatApplications/thesis/MyApplications/Input/trueanomaly_inputs_"+body+savestring+".txt"
