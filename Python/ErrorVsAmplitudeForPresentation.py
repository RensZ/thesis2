...
for p in range(no_p):
    ...
    for m in range(no_m):
        ...
        plt.hlines(formalErrorLevel, np.min(amplitudes_relative)-1.0e-4, np.max(amplitudes_relative)+0.5, color=cmap(0), linestyle='--', linewidth=1.0)
...
plt.tight_layout()
plt.savefig(dir_plots+"parameter_errors_vs_amplitude_forpresentation.png")
...
for p in range(no_p):
    ...
    for m in range(no_m):
        formalErrorLevel = np.mean(formalErrorArray[m,:,p])
        plt.plot(amplitudes_relative, np.abs(trueErrorArray[m,:,p])/formalErrorLevel, linestyle='-', marker='o', linewidth=0.75, markersize=5.0)
...
plt.tight_layout()
plt.savefig(dir_plots+"parameter_truetofalseratio_vs_amplitude.png")
