...
//    scenarioPairs.push_back(std::make_pair(1,3));
    ...
    std::vector< TranslationalPropagatorType > propagatorOptions;
    propagatorOptions.push_back(cowell);
    propagatorOptions.push_back(encke);
    propagatorOptions.push_back(gauss_keplerian);
    propagatorOptions.push_back(gauss_modified_equinoctial);

    for (unsigned int p = 0; p<propagatorOptions.size(); p++){
    ...
    //        std::cout<<"input values imported: "<<json_input<<std::endl;
            ...
            if (propagatorOptions.at(p) != cowell ){
        ...
        //        double J2amplitude = 0.005E-7 / calculateLegendreGeodesyNormalizationFactor(2,0); // currently from Antia et al 2008
            ...
            std::string outfile1 = outputSubFolder + "/StatePropagationHistoryBackwards_Integrator" + integratorString + "_Propagator" + std::to_string(p) + ".dat";
            std::string outfile2 = outputSubFolder + "/StatePropagationHistory_Integrator" + integratorString + "_Propagator" + std::to_string(p) + ".dat";
...
//            } else{
                ...
                propagatorSettings = std::make_shared< TranslationalStatePropagatorSettings< long double > >
                        ( centralBodies, accelerationModelMap, bodiesToPropagate,
                          systemInitialState, terminationSettings, propagatorOptions.at(p)); //, dependentVariablesToSave);
...
//            }
            ...
            for( unsigned int i = 0; i < bodiesToPropagate.size(); i++ )
            {
                ...
                input_output::writeDataMapToTextFile(
                            ...
                            "/StatePropagationHistory_Integrator" + integratorString + "_Propagator" + std::to_string(p) + ".dat",
                            outputSubFolder,
                            "",
...
//            } else{
                ...
                backwardPropagatorSettings = std::make_shared< TranslationalStatePropagatorSettings< long double > >
                        ( centralBodies, accelerationModelMap, bodiesToPropagate,
                          systemFinalState, backwardTerminationSettings, propagatorOptions.at(p));
...
//            }
            ...
            for( unsigned int i = 0; i < bodiesToPropagate.size(); i++ )
            {
                ...
                input_output::writeDataMapToTextFile(
                            ...
                            "/StatePropagationHistoryBackwards_Integrator" + integratorString + "_Propagator" + std::to_string(p) + ".dat",
                            outputSubFolder,
                            "",
                            std::numeric_limits< long double >::digits10,
                            std::numeric_limits< long double >::digits10,
