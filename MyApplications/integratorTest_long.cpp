...
//    scenarioPairs.push_back(std::make_pair(1,3));
    ...
    // for 12 hours to 4 hours, use intervals of 30 minutes
    for(double dt=12.0*oneHour; dt>4.0*oneHour; dt-=1800.0){
        std::cout<<dt<<" ";
        stepSizes.push_back(dt);
    }

    // for 4 hours to 1 hour, use intervals of 10 minutes
    for(double dt=4.0*oneHour; dt>oneHour; dt-=600.0){
        std::cout<<dt<<" ";
        stepSizes.push_back(dt);
    }

    // for 1 hour to 30 min, use intervals of 5 minutes
    for(double dt=oneHour; dt>oneHour/2.0; dt-=300.0){
        std::cout<<dt<<" ";
        stepSizes.push_back(dt);
    }

    // for 30 min to 10 min, use intervals of 2.5 minutes
    for(double dt=oneHour/2.0; dt>599.0; dt-=150.0){
        std::cout<<dt<<" ";
        stepSizes.push_back(dt);
    }
...
//    stepSizes.push_back(14400.0); // 240 min / 4 hours

//    stepSizes.push_back(10800.0); // 180 min / 3 hours
//    stepSizes.push_back(7200.0); // 120 min / 2 hours
//    stepSizes.push_back(6300.0); // 105 min
//    stepSizes.push_back(5400.0); // 90 min / 1.5 hours
//    stepSizes.push_back(3600.0); // 60 min / 1 hour
//    stepSizes.push_back(2700.0); // 45 min
//    stepSizes.push_back(1800.0); // 30 min
//    stepSizes.push_back(1500.0); // 25 min
//    stepSizes.push_back(1200.0); // 20 min
//    stepSizes.push_back(900.0); // 15 min
//    stepSizes.push_back(600.0); // 10 min
//    stepSizes.push_back(300.0); // 5 min

    std::vector< std::string > integratorStrings;
    ...
    std::cout<<"precision: "<<std::numeric_limits< double >::digits10<<std::numeric_limits< long double >::digits10<<std::endl;

    for (unsigned int s = 0; s<stepSizes.size(); s++){
        for (unsigned int in = 0; in<integratorStrings.size(); in++){

            std::string input_filename = "inputs_multiplemissions_Fienga2019.json";
    ...
    //        std::cout<<"input values imported: "<<json_input<<std::endl;
            ...
            // Scenario
            const int realityScenario = 4;
            const int estimationScenario = 4;
            ...
            std::string outputSubFolder = getOutputPath( ) + "integratorTest_long";
        ...
        //        double J2amplitude = 0.005E-7 / calculateLegendreGeodesyNormalizationFactor(2,0); // currently from Antia et al 2008
            ...
            double initialTimeStep = stepSizes.at(s);
            double minimumStepSize = initialTimeStep;
            double maximumStepSize = initialTimeStep;

                        std::cout<<"---- RUNNING SIMULATION FOR INTEGRATOR "<<integratorStrings.at(in)
                                 <<" AND STEP SIZE "<<initialTimeStep<<" ----"<<std::endl;
            ...
            if (integratorStrings.at(in) == "ABM" || integratorStrings.at(in) == "A12" ){
                ...
                std::cout<<"integrator order: "<<maximumOrder<<std::endl;
            ...
            if (integratorStrings.at(in) == "RK4"){
                integratorSettings =
                        std::make_shared< IntegratorSettings< > >
                        ( rungeKutta4, initialSimulationTime, initialTimeStep );
                backwardIntegratorSettings =
                        std::make_shared< IntegratorSettings< > >
                        ( rungeKutta4, finalSimulationTime, -1.0*initialTimeStep );
            }
            if (integratorStrings.at(in) == "RK7"){
                integratorSettings =
            ...
            std::string integratorString = integratorStrings.at(in) + std::to_string(static_cast<int>(initialTimeStep));

            std::string outfile1 = outputSubFolder + "/StatePropagationHistoryBackwards" + integratorString + ".dat";
            std::string outfile2 = outputSubFolder + "/StatePropagationHistory" + integratorString + ".dat";

            std::cout<<"checking if files are present:"<<std::endl;
            std::cout<<"    "<<outfile1<<std::endl<<"    "<<outfile2<<std::endl;

            std::ifstream infile1(outfile1);
            std::ifstream infile2(outfile2);

            if (infile1.good() && infile2.good()){
                std::cout<<"following files already present, moving on"<<std::endl;
                continue;
            }
...
//            typedef Eigen::Matrix<long double, Eigen::Dynamic, 1> Eigen::Matrix<long double, Eigen::Dynamic, 1>;
            ...
            std::vector< std::map< double, Eigen::Matrix<long double, Eigen::Dynamic, 1> > > spiceStatesAtPropagationTimes;
...
//                std::vector< std::map< long double, Eigen::Matrix<long double, Eigen::Dynamic, 1> > > integrationResult = dynamicsSimulator.getEquationsOfMotionNumericalSolution( );
...
//                dependentVariablesHistory = dynamicsSimulator.getDependentVariableHistory();
                     ...
                     stateIterator != integrationResult.end( ); stateIterator++ )
                {
                    for( unsigned int i = 0; i < bodiesToPropagate.size(); i++ )
                    {
                        allBodiesPropagationHistory[ i ][ stateIterator->first ] = stateIterator->second.segment( i * 6, 3 );
//                        spiceStatesAtPropagationTimes[ i ][ stateIterator->first ] =
//                                getBodyCartesianStateAtEpoch(bodiesToPropagate.at( i ),"SSB","ECLIPJ2000","None",stateIterator->first);
...
//            }
            ...
            for( unsigned int i = 0; i < bodiesToPropagate.size(); i++ )
            {
                ...
                input_output::writeDataMapToTextFile(
                            ...
                            "StatePropagationHistory" + integratorString + ".dat",
                            outputSubFolder,
                            "",
...
//                            "spiceStatesAtPropagationTimes" + integratorString + ".dat",
//                            outputSubFolder,
//                            "",
...
//            } else{
                ...
                SingleArcDynamicsSimulator <long double> backwardDynamicsSimulator (bodyMap,backwardIntegratorSettings,backwardPropagatorSettings,true,false,true);
                std::map< double, Eigen::Matrix<long double, Eigen::Dynamic, 1> > backwardIntegrationResult = backwardDynamicsSimulator.getEquationsOfMotionNumericalSolution( );
                     ...
                     stateIterator != backwardIntegrationResult.end( ); stateIterator++ )
                {
                    for( unsigned int i = 0; i < bodiesToPropagate.size(); i++ )
                    {
                        allBodiesBackwardPropagationHistory[ i ][ stateIterator->first ] = stateIterator->second.segment( i * 6, 3 );
...
//            }
            ...
            for( unsigned int i = 0; i < bodiesToPropagate.size(); i++ )
            {
                ...
                input_output::writeDataMapToTextFile(
                            ...
                            "StatePropagationHistoryBackwards" + integratorString + ".dat",
                            outputSubFolder,
                            "",
                            std::numeric_limits< long double >::digits10,
                            std::numeric_limits< long double >::digits10,
