...
//            spice_interface::loadStandardSpiceKernels( );
    ...
    double observationReductionFactor = 1.0; //decreases amount of observations by a factor n to speed up the algorithm considerably

    // Different test setups, all bools should be false for actual runs
    ...
    double solarMinimumEpoch = secondsAfterJ2000(solarMinimumDatetime);
...
//    filenames.push_back("inputs_multiplemissions_ZeroAmplitude.json");
//    filenames.push_back("inputs_multiplemissions_VeryLargeAmplitude.json");
//    filenames.push_back("inputs_multiplemissions_Ajab2008.json");
...
//                }
        ...
        //        std::cout<<"input values imported: "<<json_input<<std::endl;
                ...
                if (testCeres){outputSubFolder += "_testCeres";}
                if (onlyUseFirst4Asteroids){outputSubFolder += "_onlyUseFirst4Asteroids";}
                if (testWithoutFlybys){outputSubFolder += "_testWithoutFlybys";}
                if (testDifferentSolarMinimum){
                    outputSubFolder += "_testSolarMinimumPlus" + std::to_string(differentSolarMinimum);
...
//                    relativity::variableJ2Interface->setMeanJ2(valuesSunGravitationalMoments(2));
    ...
    //            }
                ...
                if (testWithoutEstimatingJ2 == false){
                ...
                if (includeTimeVaryingGravitationalMomentsSunInEstimation){
                    ...
                    if (estimationScenario >= 2){
                        ...
                        if (estimateJ2Amplitude){
                            ...
                            if (sigmaJ2Amplitude == 0.0){sigmaJ2Amplitude = 1E-7;}
    ...
    //            noiseFunctions[ n_way_range ] = mercuryOrbiterNoiseFunction;
            ...
            //        Eigen::Vector3d constantSatelliteError; constantSatelliteError << 10.0, 10.0, 10.0;
                ...
                if (estimationScenario < 3){
                    relativity::variableJ2Interface->setAmplitude(0.0);
                }
